...
class TestNode(unittest.TestCase):
    ...
    def test_integer_arguments(self):
        result = funstash.fun(operator.add, 2, 3).compute()
        self.assertEqual(result, 5)

    def test_node_arguments(self):
        result = funstash.fun(operator.add,
                               funstash.fun(lambda: 2),
                               funstash.fun(lambda: 3),
                               ).compute()
        self.assertEqual(result, 5)

    def test_integer_keywordarguments(self):
        result = funstash.fun(_add_xy, x=2, y=3).compute()
        self.assertEqual(result, 5)

    def test_node_keywordarguments(self):
        result = funstash.fun(_add_xy,
                               x=funstash.fun(lambda: 2),
                               y=funstash.fun(lambda: 3),
                               ).compute()
        self.assertEqual(result, 5)

    def test_path_arguments(self):
        node = funstash.fun(_add_xy,
                             funstash.fun(_mul_xy, 1, 2),
                             funstash.fun(_mul_xy, 3, 4),
                             )
    ...
    def test_path_kwarguments(self):
        node = funstash.fun(_add_xy,
                             x=funstash.fun(_mul_xy, x=1, y=2),
                             y=funstash.fun(_mul_xy, x=3, y=4),
                             )
    ...
    def test_integer_mixedarguments(self):
        result = funstash.fun(_add_xy, 2, y=3).compute()
        self.assertEqual(result, 5)
    ...
    def test_cache(self):
        ...
        node = funstash.fun(_slow_identity, 5, waitseconds=1)
    ...
    def test_modify_arguments(self):
        node = funstash.fun(operator.add, 2, 3)
    ...
    def test_modify_nonexistant_kwargument_raises_keyerror(self):
        node = funstash.fun(_add_xy, x=2, y=3)
        with self.assertRaises(KeyError):
            node["z"] = 4
    ...
    def test_modify_nodearguments(self):
        node = funstash.fun(operator.add,
                             funstash.fun(lambda: 2),
                             funstash.fun(lambda: 3)
                             )
        result1 = node.compute()
        node[1] = funstash.fun(lambda: 4)
        result2 = node.compute()
        self.assertEqual(result1, 5)
        self.assertEqual(result2, 6)
    ...
    def test_modify_path_arguments(self):
        node = funstash.fun(_add_xy,
                             funstash.fun(_mul_xy, 1, y=2),
                             y=funstash.fun(_mul_xy, 3, y=4),
                             )
        node["0/0"] = 10
        node["0/y"] = 20
        node["y/0"] = 30
    ...
    def test_pickleable(self):
        node1 = funstash.fun(_add_xy, x=2, y=3)
        node2 = pickle.loads(pickle.dumps(node1))
        self.assertEqual(node1.compute(), node2.compute())

    def test_cloudpickle(self):
        node1 = funstash.fun(_add_xy, x=funstash.fun(lambda: 2), y=3)
    ...
    def test_scan_oneargument(self):
        node = funstash.fun(operator.mul, 2, 2)
    ...
    def test_scan_onekwargument(self):
        node = funstash.fun(_mul_xy, x=2, y=2)
    ...
    def test_clone_reuses_cache(self):
        cachedir = tempfile.mkdtemp()
        node = funstash.fun(operator.add,
                             funstash.fun(_slow_identity, 2, waitseconds=1),
                             funstash.fun(_slow_identity, 3, waitseconds=1),
                             )
        clone = node.clone()
